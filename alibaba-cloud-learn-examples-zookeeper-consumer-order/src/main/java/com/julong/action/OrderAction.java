...
public class OrderAction {
	...
	public CommonResult<PaymentInfo> getPayment(@PathVariable("paymentId") long paymentId){
		return this.restTemplateImpl.getForObject("http://alibaba-cloud-learn-examples-zookeeper-provider-payment/payment/get/"+paymentId, CommonResult.class);
	}
	
	@SuppressWarnings("unchecked")
	@PostMapping(value="/consumer/save")
	public CommonResult<PaymentInfo> savePayment(@RequestBody PaymentInfo paymentInfo){
		return this.restTemplateImpl.postForObject("http://localhost:8004/payment/save",paymentInfo, CommonResult.class);
	}
}
